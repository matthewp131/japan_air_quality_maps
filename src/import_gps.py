...
def main():
    ...
    parser.add_argument(
        "stations_xlsx",
        help="a XLSX Workbook with a Stations sheet produced by address_to_gps.py",
    )
    parser.add_argument(
        "input_sheet", help="sheet name inside XLSX file, probably Stations"
    )
    ...
    for geocoding_result in geocoding_results:
        df.loc[df[station_code] == geocoding_result["station_code"], "latitude"] = (
            geocoding_result["response"]["geometry"]["location"]["lat"]
        )
        df.loc[df[station_code] == geocoding_result["station_code"], "longitude"] = (
            geocoding_result["response"]["geometry"]["location"]["lng"]
        )

    print(df[[station_code, "latitude", "longitude"]].head())

    with pandas.ExcelWriter(
            args.stations_xlsx, if_sheet_exists="overlay", mode="a"
    ) as excel_writer:
