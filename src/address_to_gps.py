...
def main():
    ...
    parser.add_argument(
        "input_file",
        help="a TMYYYY0000.txt file with Shift-JIS CSV of measuring station addresses",
    )
    ...
    headers = {"prefecture": "都道府県名", "city": "市区町村名", "address": "住所"}

    df = pandas.read_csv(args.input_file, encoding="shift_jisx0213")

    df["full_address"] = (
            df[headers["prefecture"]].astype(str)
            + df[headers["city"]].astype(str)
            + df[headers["address"]].astype(str)
    )
    ...
    if os.path.isfile(args.output_file):
        with pandas.ExcelWriter(
                args.output_file, if_sheet_exists="overlay", mode="a"
        ) as excel_writer:
